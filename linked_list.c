...
TList* getPersonality(FILE* f) {
  ...
  while(fgets(info, 500, f)) {
    ...
    node->next = new_node;
    node = new_node;
    for(int k = 0; k < 150; k++) name[k] = '\0';
    for(int k = 0; k < 600; k++) def[k] = '\0';
  }
...
TList* getDatePersonality(FILE *f) {
  ...
  while(fgets(info, 500, f)) {
    ...
    i = 0; 
    int flag = 0; // flag == 0 --> reading name | flag == 1 --> reading date of birth | flag == 2 --> reading date of death
    int temp;
    char name[100];
...
// Takes as an input a Linked list and a node with the date_of_birth in it. We have to fill in the rest of the gaps
...
int compareAge(TList* n1, TList* n2) {
  int age1 = (n1->DoD[6]-'0')*1000 + (n1->DoD[7]-'0')*100 + (n1->DoD[8]-'0')*10 + (n1->DoD[9]-'0') - (n1->DoB[0]-'0')*1000 - (n1->DoB[1]-'0')*100 - (n1->DoB[2]-'0')*10 - (n1->DoB[3]-'0');
  int age2 = (n2->DoD[6]-'0')*1000 + (n2->DoD[7]-'0')*100 + (n2->DoD[8]-'0')*10 + (n2->DoD[9]-'0') - (n2->DoB[0]-'0')*1000 - (n2->DoB[1]-'0')*100 - (n2->DoB[2]-'0')*10 - (n2->DoB[3]-'0');
  return age1 > age2;
}
TList* split(TList* head) { // returns a pointer to the head of the second linked list
...
TList* merge(TList* head1, TList* head2, int (*compare)(TList*, TList*)) {
    ...
    TList* node2 = head2;
    TList* head;
    if(compare(node1, node2)){head = node1; node1 = node1->next;}
    else {head = node2; node2 = node2->next;}
    TList* new_head = head;
    while(node1 != NULL && node2 != NULL) {
        if(compare(node1, node2)){new_head->next = node1; node1 = node1->next;}
        else {new_head->next = node2; node2 = node2->next;}
